...
function contact_us()
{
    ...
    if ($sql_query && $sql_query->num_rows > 0) {
        ...
        return [
            ...
            'devto' => $result['devto'] ?? '',
            'hashnode' => $result['hashnode'] ?? '',
            'discord' => $result['discord'] ?? '',
            'producthunt' => $result['producthunt'] ?? '',
            'google_review' => $result['google_review'] ?? '',
            'map' => $result['map'] ?? ''
        ];
    }
...
function get_blog_home()
{
    ...
    $sql_blog = "SELECT * FROM `blogs` order by id desc limit 3";
...
function get_blog($limit)
{
    ...
    $sql_blog = "SELECT * FROM `blogs` order by created_at desc limit $limit";
    $res_blog = mysqli_query($conn, $sql_blog);
...
function get_portfolio(){
    ...
    $sql_foot = "SELECT * FROM `products` WHERE `pro_cate` = '30797' order by id desc LIMIT 20";
...
function get_all_blog_tags($top = 15)
{
    ...
    $sql = "SELECT `tags` FROM `blogs` WHERE `status` = 'published' AND `tags` != ''";
    $res = mysqli_query($conn, $sql);
...
// ── Real portfolio project count (same filter as get_portfolio) ───────────
function count_portfolio_projects()
{
    global $conn;

    $sql = "SELECT COUNT(*) AS total FROM `products` WHERE `pro_cate` = '30797'";
    $res = mysqli_query($conn, $sql);
...
function average_client_rating()
{
    global $conn;

    $sql = "SELECT AVG(rating) AS avg_rating, COUNT(*) AS total FROM `testimonials` WHERE rating IS NOT NULL";
    $res = mysqli_query($conn, $sql);
    ...
    $row = mysqli_fetch_assoc($res);
    return [
        'avg' => $row['total'] > 0 ? round($row['avg_rating'], 1) : 0,
