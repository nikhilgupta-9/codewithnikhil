...
<head>
  ...
  <style>
    ...
    /* ---- Portfolio Cards ---- */
    /* .portfolio-col  = outer Bootstrap column (used by JS filter) */
    /* .portfolio-item = inner card (used for hover/visual styling)  */
    .portfolio-item {
      border-radius: 12px;
      overflow: hidden;
...
<body class="homepage4-body">
  ...
  <div class="portfolio-section-area sp1 bg2">
    <div class="container">
      ...
      while ($sc = mysqli_fetch_assoc($res_cats)) {
          ...
          $subCatName[$sc['slug_url']]     = $sc['categories'];
      }
      ?>
      ...
      <div class="row portfolio-container" id="portfolioGrid">
        <?php if (!empty($portfolios)): ?>
          <?php foreach ($portfolios as $portfolio):
             ...
             */
            $category    = $subCatMap[$portfolio['pro_sub_cate'] ?? ''] ?? 'other';
              ...
              <div class="portfolio-img">
                ...
                <div class="portfolio-overlay">
                  ...
                  <a href="<?= htmlspecialchars($portfolio['slug_url']) ?>"
                     target="_blank" rel="noopener noreferrer" title="Visit Live Site">
  ...
  <script>
    $(document).ready(function () {
      ...
      /*
       * FIX: Target .portfolio-col (the outer column div) — not the inner card.
       * show()/hide() sets display:none instantly, so Bootstrap row
       * collapses the space immediately with no empty gaps.
       */
      $('.portfolio-filter button').on('click', function () {
        ...
        if (filter === 'all') {
          ...
          $('.portfolio-col').hide();
          var $matches = $('.portfolio-col[data-category="' + filter + '"]');
          $matches.show();
